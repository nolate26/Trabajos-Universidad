...
int main(int argc, char** argv) {
	...
	Kid kid = {
	    .name = "Roberto",
	    .dog = {
	        // Dog es un struct, asi que seguimos
	        .name = "Spotty",
	        .age = 5,
	    }};
	...
	Adult* adultos = calloc(2, sizeof(Adult));
	...
	// Ahora debemos inicializar cada uno de los perros. Esto se puede hacer de
	// forma directa
	adult2.dogs[0] = (Dog){.name = "Apollo", .age = 2};
	adult2.dogs[1] = (Dog){.name = "Baggy", .age = 7};
	adult2.dogs[2] = (Dog){.name = "Kiki", .age = 12};

	adultos[0] = adult;
	adultos[1] = adult2;

	printf("nombre: %s\n", adultos[0].dogs[1].name);
	printf("nombre: %s", adultos[1].name);
