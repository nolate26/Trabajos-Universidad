...
class NodoAmigoSecreto:
    ...
    def insertar_amigo_secreto(self, nuevo_nodo, posicion,
        ...
        # Completar
        # si es posicion 0
        nodo = NodoAmigoSecreto(nuevo_nodo)
        for _ in range(posicion_actual):
            self.usuario = self.usuario.siguiente
        if posicion == 0:
            nodo.siguiente  = self.usuario
            self.usuario = nodo
        for _ in range(posicion):
            self.usuario = self.usuario.siguiente
        nodo.siguiente = self.usuario.siguiente 
        self.usuario.siguiente = nodo
