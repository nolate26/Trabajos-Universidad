...
class Usuario:
    ...
    def ver_videos(self, nombre_lista):
        l_videos = self.listas_reproduccion[nombre_lista]
        for pelicula in ListaReproduccion(l_videos,self, self.nombre):
            pelicula.reproducir()
            calificacion = self.calificar_pelicula(pelicula)
            yield (calificacion, pelicula.rankings)
