...
class Cliente:
    ...
    def escuchar(self):
        thread_escuchar = threading.Thread(target=self.thread_escuchar)
        thread_escuchar.start()


    def thread_escuchar(self): #revisar
        while True:
            ...
            while len(bytes_mensaje) < largo_mensaje:
                ...
                bytes_mensaje.extend(self.socket_cliente.recv(leer_mensaje))
                mensaje = self.decodificar_mensaje(bytes_mensaje)
                self.manejar_comando(mensaje)

    def enviar(self, msg):
        mensaje_of = self.codificar_mensaje(msg)
        msg_largo = len(mensaje_of).to_bytes(4, byteorder="big")
        self.socket_cliente.send(msg_largo + mensaje_of)
        self.evento_prints.set()
